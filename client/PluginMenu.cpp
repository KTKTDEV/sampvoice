    ...
    const Resource& const rShader, const Resource& const rLogo, const Resource& const rFont) noexcept
{
    ...
    std::string fontPath = GetWindowsFontPath("upclb.ttf"); // ใช้ฟอนต์ไทยที่คุณเลือก
...

    {
        ...
        PluginMenu::pTitleFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
            fontPath.c_str(),
            varTitleFontSize,
            NULL,
...

    {
        ...
        PluginMenu::pTabFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
            fontPath.c_str(),
            varTabFontSize,
            NULL,
            ImGui::GetIO().Fonts->GetGlyphRangesThai()
...

    {
        ...
        if (!Render::ConvertBaseYValueToScreenYValue(kBaseFontDescSize, varDescFontSize))
        {
         ...
         PluginMenu::pDescFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
            fontPath.c_str(),
             varDescFontSize,
            NULL,
            ImGui::GetIO().Fonts->GetGlyphRangesThai()
...

    {
        ...
        PluginMenu::pDefFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
            fontPath.c_str(),
            varFontSize,
            NULL,
            ImGui::GetIO().Fonts->GetGlyphRangesThai()
                    ...
                    ImGuiWindowFlags_NoResize))
                {
                    ...
                    for (const auto& playerInfo : blackList)
                    {
                        ...
                        if (ImGui::Button("##label", { listWidth, ImGui::GetFontSize() + 2.f }))
                        {
                            ...
                            playerToUnlock = playerInfo.playerName; // เก็บชื่อผู้เล่นที่จะปลดบล็อก
                            playerRemoved = true;
                        }

                        ImGui::PopID();

                        if (playerRemoved) break;
